...
func NewEndpoints(service pb.StoragesvcServer) svc.Endpoints {
	// Business domain.
	...
	// Endpoint domain.
	var (
		imageurlEndpoint = svc.MakeImageUrlEndpoint(service)
		fuploadEndpoint  = svc.MakeFUploadEndpoint(service)
		uploadEndpoint   = svc.MakeUploadEndpoint(service)
	)
...
func Run(cfg svc.Config) {
	...
	go func() {
		...
		s := grpc.NewServer()
		pb.RegisterStoragesvcServer(s, srv)
